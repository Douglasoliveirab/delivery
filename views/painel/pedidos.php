...
<head>
  ...
  <link rel="stylesheet" href="../assets/css/pianelcss/pedidos.css">
...
<body>
    ...
    // Busca o pedido com id 3
    $stmt = $conn->prepare("SELECT pedidos.*, clientes.nome, clientes.sobrenome, clientes.email, clientes.telefone, clientes.endereco, GROUP_CONCAT(DISTINCT CONCAT(produtos.nome_produto, ' QTD: ', itens_pedido.quantidade) SEPARATOR ', ') AS produtos_e_quantidades, SUM(itens_pedido.quantidade) AS total_quantidade, SUM(produtos.valor * itens_pedido.quantidade) AS total_valor FROM pedidos JOIN clientes ON pedidos.id_cliente = clientes.id_cliente JOIN itens_pedido ON pedidos.id_pedido = itens_pedido.id_pedido JOIN produtos ON itens_pedido.id_produto = produtos.id_produto GROUP BY pedidos.id_pedido, clientes.nome, clientes.sobrenome, clientes.email, clientes.telefone, clientes.endereco;");
    $stmt->execute();
    ...
    if (isset($_POST['alterar_status'])) {
      ...
      $stmt = $conn->prepare("UPDATE pedidos SET status = :novo_status WHERE id_pedido = :id_pedido");
