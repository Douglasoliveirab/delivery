...
if (isset($_GET['habilitar'])) {
    ...
    $update_produto = $conexao->prepare("UPDATE produtos SET status_produto = 'ativo' WHERE id_produto = ?");
    $update_produto->bindParam(1, $id, PDO::PARAM_INT);
    $update_produto->execute();
    header("Location: all_produtos.php");
    exit();
...
if (isset($_GET['todos'])) {
    ...
    $select = $conexao->prepare("SELECT * FROM produtos WHERE  status_produto = 'inativo'");
    $select->execute();
    $produtos = $select->fetchAll();
} else {
    $select = $conexao->prepare("SELECT * FROM produtos WHERE  status_produto = 'ativo'");
...
<ol class="breadcrumb">
    ...
    if (isset($_SESSION['previlegios']) && $_SESSION['previlegios'] == 'admin') {
        echo  '<li>
        <button id="btn-cadastrar-banner" class="btn btn-default">Cadastrar Produto</button>
    </li>';
    }
    ?>
    <li>
    ...
    <li>
        <a href="?inativos" class="btn btn-default">Mostrar Produtos Desabilitados</a>
    </li>
                            ...
                            foreach ($produtos as $produto) {
                                ...
                                if (isset($_GET['todos'])) {
                                    if ($produto['status_produto'] == 'inativo') {
                                        echo '<a href="atualizar_produto.php?id=' . $produto['id_produto'] . '" class="btn btn-primary btn-xs btn-flat">Editar</a>
                                             <a href="?habilitar=' . $produto['id_produto'] . '" class="btn btn-success btn-xs btn-flat">Habilitar</a>';
                                    } elseif ($produto['status_produto'] == 'ativo') {
                                        echo '<a href="atualizar_produto.php?id=' . $produto['id_produto'] . '" class="btn btn-primary btn-xs btn-flat">Editar</a>
                                              <a href="?delete=' . $produto['id_produto'] . '" class="btn btn-danger btn-xs btn-flat">Excluir</a>';
                                    }
                                } else {
